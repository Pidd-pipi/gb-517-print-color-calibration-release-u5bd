...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	printRunService := service.NewPrintRunService(printRunRepository, releaseDecisionRepository, unitOfWork, securityService)
	colorProofService := service.NewColorProofService(colorProofRepository, releaseDecisionRepository, unitOfWork, securityService)
	releaseDecisionService := service.NewReleaseDecisionService(releaseDecisionRepository, unitOfWork, securityService)
